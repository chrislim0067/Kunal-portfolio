export const SplitT = function (element, options) {
  ...
  const searchTextNodes = function (element) {
    var foundTextNodes = [];
  ...
  const createElement = function (text, relatedNode) {
    var node = document.createElement("div") as any;
  ...
  const splitCharacters = function (textNode) {
  ...
  var splitWords = function (textNode) {
    ...
    for (var w = 0; w <= textWords.length - 1; w++) {
      ...
      if (w < textWords.length - 1) createElement(" ", textNode); //spacing for word
    }
    textNode.parentNode.removeChild(textNode);
    return words;
  };
  const splitTextNodes = function (textNodes) {
      ...
      if (options.words == 0) {
        ...
        if (options.chars == 1) {
          for (var w = 0; w <= words.length - 1; w++) {
            let word = words[w];
            var chars = splitCharacters(word.nodeText);
